...
to create-or-remove-swimmers
  ...
  if current-swimmers < numberSwimmersInPool and any? available-patches [       ; If there are less swimmers than desired, and there are available patches then:
    ...
    splitSwimmers ; split the swimmers between the two pools
    print(num_pool_one)
    print(num_pool_two)
...
;      ]
    ...
    if allocation = "logic" [                                                   ; logically allocates swimmer location
      logicalAllocation 0 (num_pool_one - 1) 0 numberOfLanes                    ; place in pool 1
      if numberLanesPool2 > 0 [logicalAllocation num_pool_one (num_pool_one + num_pool_two - 1) numberOfLanes (numberOfLanes + numberLanesPool2)] ; place in pool 2, if pool 2 exists
    ...
    if allocation  = "random" [                                ; allocates the swimmers randomly based on the concept of looping through the lanes, and chosing a random swimmer to go into that lane
      randomAllocation 0 (numberOfLanes) num_pool_one           ; allocate to pool one
      if numberLanesPool2 > 0 [randomAllocation numberOfLanes (numberOfLanes + numberLanesPool2) num_pool_two]
    ]
...
to splitSwimmers
  set num_pool_one floor((count turtles) / 2) + count turtles mod 2                                 ; Start by assigning half and half but ensuring both are integers
  set num_pool_two floor((count turtles) / 2)
  set max_size_one 2 *(ceiling (poolLength / 2) * numberOfLanes)        ; Define the maximun for each lane
  set max_size_two 2 *(ceiling (lengthOfPool2 / 2) * numberLanesPool2)
  ifelse num_pool_one > max_size_one [                                 ; if pool one is above capacity
    set num_pool_two num_pool_two + num_pool_one - max_size_one        ;then add the surplus swimmers to pool two
    set num_pool_one max_size_one                                      ; and set the number of swimmers in pool one to be the maximun possible
  ] [                                                                  ; repeat for if pool two is above capacity. Note: it shouldn't be possible for both to be above capacity, because of earlier error detection
    if num_pool_two > max_size_two [
      set num_pool_one num_pool_one + num_pool_two - max_size_two
      set num_pool_two max_size_two
    ]
  ]
end

to logicalAllocation[start-swimmer end-swimmer start-lane end-lane]
  let available-patches patches with [pcolor != (rgb 0 0 0) and pycor mod 4 != 2 and pycor <= end-lane * 4 and pycor > start-lane * 4] ; we are looking for a patch in the pool we are currently in
  let num-lanes end-lane - start-lane
  let num-swimmers end-swimmer - start-swimmer + 1 ; we add one since the swimmers are zero indexed
  let swimmers-per-lane floor (num-swimmers / num-lanes)            ; We want equal numbers of swimmers per lane, so take the ceiling of the number of swimmers, and the number of lanes. This is the number of swimmers per lane
  let restOfSwimmers num-swimmers mod num-lanes
  let i start-swimmer                           ; i is the swimmer we are currently on
  let cur-lane start-lane                       ; start at the lane we are on
  while [i < end-swimmer and cur-lane < end-lane] [ ; while we haven't placed all the swimmers. cur-lane < end-lane is not needed, either of these would work in its own. However, I am scared to remove it, incase some random edge case breaks it.
    let j 0                         ; j is the number of swimmers currently in the lane

    ; if we have a remainder, then add an extra swimmer in the next lane, then decrement the number of swimmers by one.
    if restOfSwimmers > 0 [                          ; if there are any swimmers that will not be included by num-swimmers * num-lanes
      set swimmers-per-lane swimmers-per-lane + 1    ; then we add one extra one in this lane
    ]
    while [j < swimmers-per-lane] [                  ; we add swimmers-per-lane number of swimmers to the lane
      let random-patch one-of available-patches with [pycor > 0 + cur-lane * 4 and pycor <= (cur-lane + 1) * 4 and (count [turtles-at 0 0] of patch pxcor pycor) = 0] ; select a random patch from the lane we're in, being careful to to place the swimmer on another swimmer
      ask turtles with [index-speed = i] [                      ; We are looking at the swimmer that is associated with our index (since we are placing them from slowest to fastest)
        ...
        set ybottom 1 + (floor (ycor / 4) )* 4                   ; calculate the ybottom value for the swimmer
        set lane-ID 1 + floor ((ybottom - 1) / 4)               ; find the lane the swimmer is in. Note: set lane-ID cur-lane + 1 ; should achieve the same effect as this
        ifelse ybottom > numberOfLanes * 4 [                    ; are we in pool one?
          set simulation-length round (item 1 lanes-length / 2) ; if so, then the lane is the length of the pool 1 lanes
        ] [                                                     ; if not, then they are the length of the pool 2 lanes
          set simulation-length round (item 0 lanes-length / 2)
        ]
      ]
      set j j + 1 ; we have added one swimmer, so increment the count of swimmers
      set i i + 1 ; and the total count of swimmmers
    ]
    if restOfSwimmers > 0 [                      ; now that we have added the extra swimmer to account of swimmers-per-lane * num-lanes < num-swimmers
      set restOfSwimmers restOfSwimmers - 1      ; we need to decrease the amount of extra swimmers needed by one
      set swimmers-per-lane swimmers-per-lane - 1 ; and reset the swimmers-per-lane value to be what it was before (floor(num-swimmers) / num-lanes)
    ]
    set cur-lane cur-lane + 1 ; after completing the above loop, we have finished looking at cur-lane, so move onto the next lane
  ]
end

to randomAllocation [start-lane end-lane num-swimmers] ; we want to call this for pool 1, and for pool 2

  let available-patches patches with [pcolor != (rgb 0 0 0) and pycor mod 4 != 2 and pycor <= end-lane * 4 and pycor > start-lane * 4] ; we are looking for a patch in the pool we are currently in



  let swimmer-index-speeds [index-speed] of turtles        ; create a list of the index-speeds of the non-allocated swimmers. This will later allow us to allocate them randomly
  let num-lanes end-lane - start-lane           ; We need to know the number of lanes
  let swimmersAdded [index-speed] of turtles with [ybottom != -1] ; This list will contain the index-speeds of the swimmers we've added already. This will allow us to reduce the swimer-index-speeds list to only contain swimmers we haven't added yet
  let i 0                                             ; set the starting index

  while [i < num-swimmers] [                               ; loop until we've added all the swimmers for the pool we're in
    ...
    let lane-number (i mod num-lanes) + 1 + start-lane                                                              ; We're looping through the lanes, so we know the lane-number we're currently on is the loop number modulus num-lanes
...
to clustering-alg
  ...
  py:set "num_pool_one" num_pool_one
  py:set "num_pool_two" num_pool_two
...
;    k_means_clustering requires 2-dimensions, so we will project the 1-D speeds onto a 2-d plane by adding 0 in the y-direction. e.g. make swimmer speed (vi, 0)
    ...
    "if num_lanes_one != 0: pool_one_info = list(cluster(swimmers[:int(num_pool_one)], num_lanes_one, max_size_one / num_lanes_one))" ; Cluster pool one
    "if num_lanes_two != 0: pool_two_info = list(cluster(swimmers[int(num_pool_one):], num_lanes_two, max_size_two / num_lanes_two))" ; Cluster pool two
  )

  ; note, the centroids of pool two will be offset by the number of lanes in pool 1
  ; note, we need to define a list of all the centroids, so that the gumbel distribution plot works properly (see comments in V2)
...
<experiments>
  ...
  <experiment name="testing_random" repetitions="5" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>setupComplete = true</exitCondition>
    <metric>swimmerSpeedsInEachLane</metric>
  ...
  <experiment name="testing_new_random_method" repetitions="5" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>setupComplete = True</exitCondition>
    <metric>minutes</metric>
  ...
  <experiment name="testing_every_allocation" repetitions="5" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>setupComplete = True</exitCondition>
    <metric>minutes</metric>
  ...
  <experiment name="testing_clustering" repetitions="5" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>setupComplete = True</exitCondition>
    <metric>swimmerSpeedsInEachLane</metric>
  ...
  <experiment name="testing_random_v5" repetitions="5" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>setupComplete = True</exitCondition>
    <metric>swimmerSpeedsInEachLane</metric>
  ...
  <experiment name="testing_logic_v5" repetitions="5" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>setupComplete = True</exitCondition>
    <metric>swimmerSpeedsInEachLane</metric>
