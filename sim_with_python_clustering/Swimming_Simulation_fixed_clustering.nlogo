...
to clustering-alg ; works when all lanes are the saem size, but not when the two pools have different legnths
...
;    k_means_clustering requires 2-dimensions, so we will project the 1-D speeds onto a 2-d plane by adding 0 in the y-direction. e.g. make swimmer speed (vi, 0)
    ...
    "for swimmer in swimmers:"                             ; loop through all of the swimmers
    ...
    "speeds = np.array(speeds)"                            ; After we complete the for loop, we've processed all teh speeds, so can now turn speeds into a numpy array
    "clf = KMeansConstrained(n_clusters=num_clusters,size_min=0,size_max=max_size,random_state=0)" ; we then perform the contrained k-means clustering algorithm on this array (again, see refernces/credit)
  )
  set centroids py:runresult "clf.fit_predict(speeds)"     ; we can grab our lsit of centroids from the result of the clustering algorithm
  let indices py:runresult "indices"                       ; and our list of indices from the execution
  let available-patches patches with [pcolor != (rgb 0 0 0) and pycor mod 4 != 2] ; then we need to define the space of which we can select tiles from. Note this is a static list, and hecne is not updated during the palcement. Hence it is only the 'available tiles' for the very first instance, and after that we reduce the numbe rof pathces we can choose from by making sure the one we select from available patches has no swimmer on it, and is in the correct lane.
  ...
  foreach indices [[index] ->                   ; We want to loop through the swimmers in a structured order. Indices is a list of index-speeds from 0-max_index_speed
    ask turtles with [index-speed = index] [    ; We place the swimmers in order of their index-speed, so loop through them in that order
      let centroid item index centroids         ; note, centroid here is effectively the lane-number - 1. i.e. each swimmer is asigned to a centroid (from 0-k), and each of these centroids reprosents a lane. So this is 0-indexed lanes
      set lane-ID centroid + 1                  ; ; The lane number of the swimmer is the centroid value + 1, since centroid is jjust lane-number zero indexed
