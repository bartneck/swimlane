...
;      ]
    ...
    if allocation  = "random" [                                ; allocates the swimmers randomly based on the concept of looping through the lanes, and chosing a random swimmer to go into that lane
      randomAllocation 0 (numberOfLanes) 0 (num_pool_one - 1)            ; allocate to pool one
      randomAllocation numberOfLanes (numberOfLanes + numberLanesPool2) (num_pool_one) (num_pool_one + num_pool_two - 1) ; allocate to pool two
    ]
...
to splitSwimmers
  set num_pool_one (count turtles) / 2                                 ; Start by assigning half and half
  set num_pool_two (count turtles) / 2
...
to randomAllocation [start-lane end-lane start-swimmer end-swimmer] ; we want to call this for pool 1, and for pool 2
  ...
  let num-swimmers end-swimmer - start-swimmer             ; the number of swimmers is the difference from start to end
  ; Note: the start-swimmer and end-swimmer values are conceptually and not literal, the next-swimmer is chosen rancomly from the once left unallocated. This is fixed in V5
  ...
  let i start-swimmer                                             ; set the starting index

  while [i <= end-swimmer] [                               ; loop until we've added all the swimmers
...
;    k_means_clustering requires 2-dimensions, so we will project the 1-D speeds onto a 2-d plane by adding 0 in the y-direction. e.g. make swimmer speed (vi, 0)
    ...
    ; Split the swimmers between pool one and pool two; this code has been relpaced by a new netlogo function, so this should be refactored eventuall.
