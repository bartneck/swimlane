...
;      ]
    ...
    if allocation = "clustering" [                     ; Assigns swimmers to lanes using a clustering algarithm
      ...
      ask turtles [                                       ; loop through the swimmers
        ...
        let max-index length centroids
        let loop-counter 0
        ...
        while [random-patch = nobody and loop-counter <= max-index] [ ; If the cluster we are supposed to be in is full
          set final-index (final-index - 1) mod max-index             ; then we move back ot the cluster below us
          set random-patch one-of available-patches with [pycor > 0 + (final-index - 1) * 4 and pycor <= final-index * 4 and (count [turtles-at 0 0] of patch pxcor pycor) = 0] ; select an availiable patch from the lane we selected in the above while loop
          set loop-counter loop-counter + 1 ; add to the loop counter. If we loop through more than the number of clusters, then there are no spaces availiable, so the pool is full, and there is an illegal amount of swimmers
        ]
